...
setwd("C:/Users/diego/Documents/Ingeniería_Ambiental/Diplomado/Modulo_2/diplo-modulo2-equipo-11")
...
#setwd("C:/Users/ferna/Documents/Diplomado_Data_Sience/Proyectos/diplo-modulo2-equipo-11/Data")
setwd("C:/Users/diego/Documents/Ingeniería_Ambiental/Diplomado/Modulo_2/diplo-modulo2-equipo-11/Data")
#setwd("/Users/bryantcanseco/Desktop/UNAM/Diplomados/Ciencia de Datos- UNAM/diplo-modulo2-equipo-11/Data")
...
aviones_sinEspeed <- aviones |> select(-speed)
vis_dat(aviones_sinEspeed)
vis_miss(aviones_sinEspeed)
...
clima_sinEwind_gust <- clima |> select(-wind_gust)
vis_miss(clima_sinEwind_gust)
```

```{r}
# AEROLINEAS
# Tipo de variables
aerolineas |> skimr::skim() |>
...
Como conclusión, podemos observar que nuestra base de datos casi no tiene valores NA. En la tabla **aerolíneas**, no tenemos ningún NA; en la tabla **aeropuertos**, el número de NAs es muy bajo, menor al **0.1%**, por lo que podríamos pasarlo por alto al tratarse de una cantidad insignificante.

En la tabla **aviones**, aunque el porcentaje total de NAs es de **11.3%**, podemos notar que estos provienen de la columna **speed**. Si eliminamos dicha columna, el número total de NAs se reduce a **0.3%**, lo cual nos permite seguir trabajando sin ningún problema.

De igual forma, en la tabla **clima**, el porcentaje más alto de NAs se encuentra en la columna **wind_gust**, con un **80%** de valores faltantes. Por lo tanto, sería conveniente eliminar esta columna, ya que no aportaría información útil al análisis. Una vez eliminada, el porcentaje de NAs en la tabla se reduce a **0.9%**, lo cual es aceptable para continuar analizando los datos.

Por último, la tabla de **vuelos** presenta una cantidad de NAs de **0.7%**, lo cual es muy bajo.

En conclusión, nuestras tablas están muy completas y no tendremos problemas al trabajar con ellas. No será necesario imputar los datos faltantes; simplemente podemos omitir esos NAs en nuestro análisis.
...
#setwd("'/Users/bryantcanseco/Desktop/UNAM/Diplomados/Ciencia de Datos- UNAM/Modulo 2/Sin título/diplo-modulo2-equipo-11'")
...
# ============================================
# Subsección 3: Analizando letras de canciones
# ============================================
```{r}
# Descomprimiendo los archivos
#utils::unzip("canciones.zip")
```

```{r}
# Preparando los dataframes de cada artista
arianagrande <- read_csv(file.path("canciones/ArianaGrande.csv"), , show_col_types = FALSE)
beyonce <- read_csv(file.path("canciones/Beyonce.csv"), show_col_types = FALSE)
billieeilish <- read_csv(file.path("canciones/BillieEilish.csv"), show_col_types = FALSE)
bts <- read_csv(file.path("canciones/BTS.csv"), show_col_types = FALSE)
drake <- read_csv(file.path("canciones/Drake.csv"), show_col_types = FALSE)
edsheeran <- read_csv(file.path("canciones/EdSheeran.csv"), show_col_types = FALSE)
eminem <- read_csv(file.path("canciones/Eminem.csv"), show_col_types = FALSE)
justinbieber <- read_csv(file.path("canciones/JustinBieber.csv"), show_col_types = FALSE)
ladygaga <- read_csv(file.path("canciones/LadyGaga.csv"), show_col_types = FALSE)
postmalone <- read_csv(file.path("canciones/PostMalone.csv"), show_col_types = FALSE)
rihanna <- read_csv(file.path("canciones/Rihanna.csv"), show_col_types = FALSE)
```

```{r}
# Cambiando la columna "Date" a formato de fecha para las actividades 1 y 2
artistas_1 <- list(
...
artistas_1 <- lapply(artistas_1, function(df){
  df |>
    dplyr::mutate(Date = as.Date(Date)) |>
    dplyr::arrange(desc(Date))
})

list2env(artistas_1, envir = .GlobalEnv)

# Cambiando la columna "Date" a formato de fecha para las actividades 3 y 4
artistas_2 <- list(
  drake = drake,
...
artistas_2 <- lapply(artistas_2, function(df){
  df |>
    dplyr::mutate(Date = as.Date(Date)) |>
    dplyr::arrange(desc(Date))
})

list2env(artistas_2, envir = .GlobalEnv)

#Cambiando la columna "Date" a formato de fecha para la activadad 5
bts <- bts |> 
  dplyr::mutate(Date = as.Date(Date)) |>
  arrange(desc(Date))
```
...
```{r}
# Definimos una función que genera la nube de palabras
generar_nube <- function(df){
  # Obteniendo el último álbum
  ultimo_album <- df |>
    dplyr::filter(!is.na(Album)) |>
    head(1) |>
    pull(Album)
  
  # Filtrando variables del último álbum
  letras <- df |>
    dplyr::filter(Album %in% ultimo_album) |>
    pull(Lyric)
  ...
  # Creando nube
  nube <- letras |>
    quanteda::tokens(remove_punt = TRUE, remove_numbers = TRUE, remove_separators = TRUE, remove_symbols = FALSE) |>
    quanteda::tokens_tolower() |>
    quanteda::tokens_remove(stopwords("english")) |>
    quanteda::dfm()
  
  par(margin = c(t = 1.5,r = 1.5, b = 1.5, l = 1.5),
      bg = "white",
      family = "Times")

  quanteda.textplots::textplot_wordcloud(nube,
                    max_words = 40,
  ...
  title(
    main = paste("Nube de palabras", ultimo_album),
    cex.main = 1.2,
    font.main = 2,
    col.main = "black",
    line = 3
...
lapply(artistas_1, generar_nube)
...
```{r}
# Obtenemos las letras combinadas
letras_combinadas <- lapply(artistas_1, function(df) {
  ultimo_album <- df |>
    dplyr::filter(!is.na(Album)) |>
    head(1) |>
    pull(Album)
...
# Creamos la nube combinada
nube_combinada <- letras_combinadas |>
  corpus() |>
  tokens(remove_punct = TRUE, remove_numbers = TRUE, remove_separators = TRUE, remove_symbols = TRUE) |>
  tokens_tolower() |>
  tokens_remove(stopwords("english")) |>
  dfm()

par(mar = c(t = 1.5,r = 1.5, b = 3.5, l = 1.5), bg = "white", family = "Times")

textplot_wordcloud(nube_combinada,
                   max_words = 100,
                   color = brewer.pal(8, "Dark2"),
                   min_count = 3,
                   scale = c(5, 0.5),
...
title(main = "Nube de palabras del último álbum\ncombinado de las cinco artistas",
      cex.main = 1.2,
      font.main = 2,
      col.main = "black",
      line = 1
...
title(
    sub = "Artistas: Ariana Grande, Beyoncé, Billie Eilish, Rihanna y Lady Gaga",
    cex.sub = 0.8,
    font.sub = 3,
    col.sub = "gray40",
    line = -1
...
```{r}
# Reutilizamos la función definida en la actividad 1
lapply(artistas_2, generar_nube)
...
```{r}
# Obtenemos las letras combinadas
letras_combinadas <- lapply(artistas_2, function(df) {
  ultimo_album <- df |>
    dplyr::filter(!is.na(Album)) |>
    head(1) |>
    pull(Album)
...
title(main = "Nube de palabras del último álbum\ncombinado de los cinco artistas",
      cex.main = 1.2,
      font.main = 2,
      col.main = "black",
      line = 1
...
title(
    sub = "Artistas: Drake, Justin Bieber, Eminem, Ed Sheeran y Post Malone",
    cex.sub = 0.8,
    font.sub = 3,
    col.sub = "gray40",
    line = -1
